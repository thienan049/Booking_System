...
ChuyenXe.updateChuyenXeById = function(MaCX, chuyenxe, result){
    sql.query("UPDATE chuyenxe SET  MaTX=?, BienSoXe=?, NgayDi=?, GioDi=? WHERE MaCX = ?", [chuyenxe.MaTX,chuyenxe.BienSoXe,chuyenxe.NgayDi,chuyenxe.GioDi,MaCX], function (err, res) {
        if(err) {
            console.log("error: ", err);
            result(null, err);
        }
